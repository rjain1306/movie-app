...
import { ErrorCreatingItem } from '../../../core/error-codes';
...
export class AuthService {
  ...
  async registerUser(
    ...
    try {
      this._logger.info(`login User with Payload: ${JSON.stringify(model)}.`);
      ...
      const outcomeUser = await this._authRepository.saveUser(user);
  ...
  async loginUser(
    ...
    try {
      ...
      const user = await this._authRepository.getByEmail(model.emailAddress);
